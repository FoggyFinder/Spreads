...
type ZipLagCursor<'K,'V,'R>(cursorFactory:Func<ICursor<'K,'V>>, lag:uint32, mapCurrentPrev:Func<'V,'V,'R>) =
  ...
  interface ICanMapSeriesValues<'K,'R> with
    member this.Map<'R2>(mapping): Series<'K,'R2> = 
      let mapCurrentPrev2 : Func<'V,'V,'R2> = Func<'V,'V,'R2>(fun c p -> mapping.ScalarFunc.Invoke(mapCurrentPrev.Invoke(c, p)))
...
type ZipLagAllowIncompleteCursor<'K,'V,'R>
  ...
  interface ICanMapSeriesValues<'K,'R> with
    member this.Map<'R2>(mapping): Series<'K,'R2> = 
      let mapCurrentPrevN2 = Func<_,_,_,_>(fun c p l -> mapping.ScalarFunc.Invoke(mapCurrentPrevN.Invoke(c, p, l)))
